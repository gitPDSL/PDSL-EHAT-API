...
export class LeaveBalanceService {
    ...
    async findById(user: string, leaveType: string, year: number, relations: string[] = []) {
        try {
            const leaveBalance = await this.leaveBalanceRepository.findOne({ where: { userId: user, leaveTypeId: leaveType, year }, relations });
            return leaveBalance;
        } catch (error) {
            if (error.name == 'ValidationError') {
                throw new BadRequestException(error.errors);
    ...
    async findOne(query: Record<string, any>) {
        try {
            ...
            const leaveBalance = await this.leaveBalanceRepository.findOne({ where: filter, relations: relations || [] });
            return leaveBalance;
        } catch (error) {
            if (error.name == 'ValidationError') {
                throw new BadRequestException(error.errors);
